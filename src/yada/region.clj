    ...
    (if encroached
      ...
      (let [border-elements
              (remove #(element/in-circum-circle? % center-coordinate)
                new-neighbors)
            borders
              (for [e border-elements
                    :let [border (element/get-common-edge @e @current)]]
        ...
        {; continue breadth-first search with neighbors in circum-circle
         ...
         :new-borders      (map :border borders)
         :edge-map-updates (map :edge-map-update borders)}))))
